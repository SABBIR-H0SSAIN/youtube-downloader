...
const VideoDownloadPreviewCard = ({
  id,
  title,
  description,
  thumbnail,
  ...
  const handleSubmit = async (e: any) => {
    ...
    const endpointUrl = `${streamEndpoint}/download/video?${params.toString()}`;
  ...
  return (
    <div
      className={cn(
        "flex flex-col bg-card gap-2 max-w-sm sm:max-w-[600px]  p-5 rounded-lg shadow-md border",
        className
      )}
    >
      ...
      <div className="flex flex-row items-center justify-center pt-3 gap-2">
        <Select name="quality" value={quality} onValueChange={setQuality}>
          <SelectTrigger className="min-w-[140px] max-w-[200px] cursor-pointer hover:ring-0 focus:ring-0 focus:ring-offset-0">
            <SelectValue placeholder="Select quality" />
          </SelectTrigger>
          <SelectContent>
            {formats.map((q: string) => (
...
export default VideoDownloadPreviewCard;
